...
@section('content')
    <div class="py-8 px-40">
        <h1 class="text-4xl font-bold text-center">Ubah</h1>
        <div class="mt-8 w-full max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-lg mx-auto">
            <form class="space-y-6" action="{{ route('mahasiswa.update', ['mahasiswa' => $mahasiswa->id]) }}" method="POST"
                autocomplete="off">
                @csrf
                @method('put')
                <div>
